...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `martincastrocm` with the `ORG` of your provider.
   - replace all instances of the `github.com/pulumi/pulumi-octopus` repository with the `REPOSITORY` location
