...
func GetLibraryVariableSetsOutput(ctx *pulumi.Context, args GetLibraryVariableSetsOutputArgs, opts ...pulumi.InvokeOption) GetLibraryVariableSetsResultOutput {
	return pulumi.ToOutputWithContext(context.Background(), args).
		ApplyT(func(v interface{}) (GetLibraryVariableSetsResultOutput, error) {
			args := v.(GetLibraryVariableSetsArgs)
			opts = internal.PkgInvokeDefaultOpts(opts)
			var rv GetLibraryVariableSetsResult
			secret, err := ctx.InvokePackageRaw("octopusdeploy:index/getLibraryVariableSets:getLibraryVariableSets", args, &rv, "", opts...)
			if err != nil {
				return GetLibraryVariableSetsResultOutput{}, err
			}

			output := pulumi.ToOutput(rv).(GetLibraryVariableSetsResultOutput)
			if secret {
				return pulumi.ToSecret(output).(GetLibraryVariableSetsResultOutput), nil
			}
			return output, nil
...
// The space ID associated with this library variable set.
func (o GetLibraryVariableSetsResultOutput) SpaceId() pulumi.StringOutput {
	return o.ApplyT(func(v GetLibraryVariableSetsResult) string { return v.SpaceId }).(pulumi.StringOutput)
}
